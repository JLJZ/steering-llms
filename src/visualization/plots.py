...
def plot_layer_sweep(
    ...
    bars = ax.bar(
        layers, gap_values, color=colors, alpha=0.8, edgecolor="none", width=0.75
    )
...
def plot_cross_effect_matrix(
    ...
    for i in range(matrix.shape[0]):
        for j in range(matrix.shape[1]):
            ...
            if i == j:
                rect = plt.Rectangle(
                    (j - 0.5, i - 0.5), 1, 1, fill=False, edgecolor="black", linewidth=2
                )
                ax.add_patch(rect)
...
def plot_evaluation_summary(results: list[dict], model_name: str) -> plt.Figure:
    ...
    h_cis = [
        r.get(
            "harmful_refusal_ci", (r["harmful_refusal_rate"], r["harmful_refusal_rate"])
        )
        for r in results
    ]
    _hbar(
    ...
    _hbar(
        ...
        None,
        "Rate",
        "False Refusal Rate (Neutral Prompts)",
        threshold=0.05,
        threshold_label="Threshold = 0.05",
        threshold_color="red",
    )
    ...
    ax_ground.axvline(
        -0.5, color="red", linewidth=1, linestyle="--", label="Heuristic threshold"
    )
    ax_ground.set_yticks(y_pos)
    ax_ground.set_yticklabels(names)
    ...
    ax_ppl.axvline(
        1.5, color="red", linewidth=1.5, linestyle="--", label="Collapse threshold"
    )
    ax_ppl.set_yticks(y_pos)
    ax_ppl.set_yticklabels(names)
    ax_ppl.set_xlabel("Perplexity ratio (steered / baseline)")
    ax_ppl.set_title("Capability: Perplexity Ratio")
    ax_ppl.legend(fontsize=9)

    fig.suptitle(
        f"Evaluation Summary — {model_name}", fontsize=14, fontweight="bold", y=1.01
    )
    fig.tight_layout()
    return fig
...
def plot_decay_analysis(
    ...
    ax.axvspan(
        0, 5, alpha=0.10, color=COLORS["targeted"], label="Critical window (t=0–5)"
    )
    ax.axhline(
        0.001,
...
def save_figure(
    fig: plt.Figure, name: str, results_dir: str, formats: tuple = ("png", "pdf")
):
    ...
    for fmt in formats:
        try:
            ...
            fig.savefig(out / f"{name}.{fmt}", dpi=150)
    plt.close(fig)
